...
spheres = [ Sphere(Vec3(-0.52, 0.0, 2.0), 0.5, Metal(Vec3(0.89, 0.65, 0.55))),
			Sphere(Vec3(0.52, 0.0, 2.0), 0.5, Metal(Vec3(0.2, 0.9, 0.55))),
			#Sphere(Vec3(0.5, -100.0, 2.0), 99.5, Metal(Vec3(1.0, 1.0, 1.0)))]
			Sphere(Vec3(0.0, -1.0, 2.0), 0.5, Metal(Vec3(1.0, 1.0, 1.0)))]

#spheres = [ Sphere(Vec3(-0.60, 0.0, 1.75), 0.5, Lambert(Vec3(0.89, 0.65, 0.55)))]
#hitable_list = Hitable_List(spheres + triangles)
#model = Model(material = Metal(Vec3(0.99, 0.83, 0.89), 0.05))
#model.readObj("../turned_box.obj")
hitable_list = Hitable_List(spheres)
#hitable_list = Hitable_List([model] + spheres)
#hitable_list = Hitable_List(randomScene(40, 4.0, 0.35, 0.4))
...
for y in range(height, 0, -1):
	for x in range(0, width):
		...
		for s in range(0, no_of_samples):
			...
			colour = colour + rayTrace(hitable_list, ray, 20)
...
ppmWriter(pixels, "random_scene_01.ppm", width, height)
